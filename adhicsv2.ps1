...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Window.Resources>
        <Style x:Key="ModernButton" TargetType="Button">
            ...
            <Setter Property="Foreground" Value="$BtnText"/>
    ...
    <Grid Margin="30" Opacity="0">
        ...
        <Border Grid.Row="1" Background="$PanelBg" CornerRadius="12" BorderBrush="$BorderColor" BorderThickness="1">
            <Grid>
                ...
                <StackPanel Grid.Row="0" Orientation="Horizontal" Margin="20,20,20,15">
                    ...
                    <TextBlock Text="Execution Summary" FontSize="16" FontWeight="Bold" Foreground="$TextPrimary" Margin="10,0,0,0" VerticalAlignment="Center"/>
                </StackPanel>
        ...
        <Grid Grid.Row="2" Margin="0,20,0,0">
            ...
            <Button Name="BtnBaseline" Grid.Column="0" Height="45" Style="{StaticResource ModernButton}">
                <StackPanel Orientation="Horizontal">
                    <TextBlock FontFamily="Segoe Fluent Icons, Segoe MDL2 Assets, Segoe UI Symbol" Text="&#xEA18;" Margin="0,0,10,0" FontSize="16" VerticalAlignment="Center"/>
                    <TextBlock Text="Enforce ADHICSv2 Security" FontWeight="SemiBold" VerticalAlignment="Center"/>
                </StackPanel>
            </Button>

            <Button Name="BtnInventory" Grid.Column="2" Height="45" Style="{StaticResource ModernButton}">
                <StackPanel Orientation="Horizontal">
                    <TextBlock FontFamily="Segoe Fluent Icons, Segoe MDL2 Assets, Segoe UI Symbol" Text="&#xE81E;" Margin="0,0,10,0" FontSize="16" VerticalAlignment="Center"/>
                    <TextBlock Text="Collect System Inventories" FontWeight="SemiBold" VerticalAlignment="Center"/>
                </StackPanel>
            </Button>

            <Button Name="BtnPass" Grid.Column="4" Height="45" Style="{StaticResource ModernButton}">
                <StackPanel Orientation="Horizontal">
                    <TextBlock FontFamily="Segoe Fluent Icons, Segoe MDL2 Assets, Segoe UI Symbol" Text="&#xE71E;" Margin="0,0,10,0" FontSize="16" VerticalAlignment="Center"/>
                    <TextBlock Text="Search Pass" FontWeight="SemiBold" VerticalAlignment="Center"/>
                </StackPanel>
            </Button>

            <Button Name="BtnUpdate" Grid.Column="6" Height="45" Style="{StaticResource ModernButton}">
                <StackPanel Orientation="Horizontal">
                    <TextBlock FontFamily="Segoe Fluent Icons, Segoe MDL2 Assets, Segoe UI Symbol" Text="&#xE895;" Margin="0,0,10,0" FontSize="16" VerticalAlignment="Center"/>
                    <TextBlock Text="Windows Update" FontWeight="SemiBold" VerticalAlignment="Center"/>
                </StackPanel>
            </Button>
        </Grid>
    </Grid>
...
function New-TaskRow {
    ...
    # Setup Initial Greyed Out State
    if ($script:IsLightMode) {
...
$script:BtnBaseline.Add_Click({
    ...
    # 1. PRE-POPULATE THE CURRENT TASK + 3 GREYED OUT
    $T_Update  = New-TaskRow "Most Recent Update Installed"
    ...
    Update-WpfUI
    Start-Sleep -Milliseconds 600 # Let user see the pending list
    ...
    if ($null -ne $LastHotfix -and $null -ne $LastHotfix.InstalledOn) {
        ...
        if ($LastHotfix.InstalledOn.Month -ne $CurrentDate.Month -or $LastHotfix.InstalledOn.Year -ne $CurrentDate.Year) {
            Complete-Task $T_Update "Most Recent Update Installed -> $($LastHotfix.HotfixID) on ($HotfixDate) - Check for updates." "Warning"
        } else {
            Complete-Task $T_Update "Most Recent Update Installed -> $($LastHotfix.HotfixID) on ($HotfixDate)" "Success" 
        }
    } else { 
        $script:BaselineSummary.Windows_Update = "Never / Unknown"
        Complete-Task $T_Update "Most Recent Update Installed -> Unknown or Never" "Warning" 
    }

    # Add next greyed task to maintain exactly 3
    ...
    # Add final greyed task
    $T_Finish = New-TaskRow "Compliance Run"
   ...
   <# 
    $T_Spooler = New-TaskRow "Allow Print Spooler Remote Connections"
    Set-TaskRunning $T_Spooler
    $RegPath = "HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Printers"
    $RegName = "RegisterSpoolerRemoteRpcEndPoint"

    # Aggressively enforce the registry key regardless of local GPO state
    Set-RegKey $RegPath $RegName 2 "Dword"
    
    # Restart the service to immediately apply the security boundary
    Restart-Service -Name "Spooler" -Force -ErrorAction SilentlyContinue | Out-Null

    $script:BaselineSummary.Spooler_Status = "Disabled"
    Complete-Task $T_Spooler "Allow Print Spooler Remote Connections -> Disabled" "Success"
...
$script:BtnInventory.Add_Click({
    ...
    $T_Finish = New-TaskRow "Finalizing and opening output directory..."
    ...
    try {
        ...
        # Enforce Accurate Windows 11 Detection Overriding Legacy WMI Strings
        ...
        # --- NEW: GET ACTIVE DOMAIN USERS / CUSTODIANS ---
        $DomainUsers = @()
        $Profiles = Get-CimInstance Win32_UserProfile -ErrorAction SilentlyContinue | Where-Object { $_.Special -eq $false }
        
        foreach ($UserProfile in $Profiles) {
            try {
                # Look up the specific account tied to the profile's SID
                $Account = Get-CimInstance Win32_UserAccount -Filter "SID='$($UserProfile.SID)'" -ErrorAction Stop
                
                # Enforce filters: Must NOT be a local account, and must NOT be disabled
                if ($Account.LocalAccount -eq $false -and $Account.Disabled -eq $false) {
                    $DomainUsers += $Account.Name
                }
            } catch {
                # Silently skip orphaned profiles where the SID no longer resolves
                continue
            }
        }
        
        # Join the array into a single comma-separated string for the CSV
    ...
    Set-TaskRunning $T_Finish
    Start-Process "explorer.exe" -ArgumentList $InventoryFolder
    Complete-Task $T_Finish "Inventory Collection Complete." "Done"
...
$script:BtnPass.Add_Click({
    ...
    # Combine them using the Windows Search OR operator
    ...
    # Get all local fixed disks (DriveType = 3)
    ...
    foreach ($Drive in $LocalDrives) {
        # Triggers native Windows File Explorer search using the combined query
        try {
